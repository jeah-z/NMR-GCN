...
def batcher():
    def batcher_dev(batch):
        graphs, labels, masks, qms = zip(*batch)
        batch_graphs = dgl.batch(graphs)
        labels = torch.stack(labels, 0)
        masks = torch.stack(masks, 0)
        qms = torch.stack(qms, 0)
        return AlchemyBatcher(graph=batch_graphs, label=labels, mask=masks, qm=qms)
...
class TencentAlchemyDataset(Dataset):
    ...
    def atom_mask(self, mol, id):
        num_atoms = mol.GetNumAtoms()
        mask = [[0]]*num_atoms
        mask[id] = [1]
        return mask

    def smi_to_dgl(self, index, self_loop=False):
        """
        ...
        solubility = float(self.target.loc[index]['NMR'])
        qm_f = float(self.target.loc[index]['QM'])
        qm = torch.FloatTensor([qm_f])
        
        l = torch.FloatTensor([solubility])
        id = int(self.target.loc[index]['id'])
        mask = self.atom_mask(mol, id)
    ...
    def _load(self):
        ...
        self.graphs, self.labels, self.masks, self.qms = [], [], [], []
        for index in self.target.index:
            ...
            self.masks.append(result[2])
            self.qms.append(result[3])
        self.normalize()
    ...
    def __getitem__(self, idx):
        g, l, mask, qm = self.graphs[idx], self.labels[idx], self.masks[idx], self.qms[idx]
        if self.transform:
            g = self.transform(g)
        return g, l, mask, qm
